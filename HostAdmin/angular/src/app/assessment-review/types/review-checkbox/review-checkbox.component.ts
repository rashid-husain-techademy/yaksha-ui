...
@Component({
  ...
  styleUrls: ['./review-checkbox.component.scss']
})
export class ReviewCheckboxComponent implements OnInit, OnDestroy {
  field: Questions;
  group: FormGroup;
  ...
  ngOnInit(): void {
    ...
    this.choices.forEach(answer => {
      let answeredChoice: UserToggledAnswers = {
        choice: answer.replace(/<p>/gi, '<p style="margin-bottom:0px">'),
    ...
    this.answers = this.answers.map(answer => answer.replace(/<p>/gi, '<p style="margin-bottom:0px">'));
